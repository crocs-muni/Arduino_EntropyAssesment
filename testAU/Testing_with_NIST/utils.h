...
bool read_file(const char* file_path, char data[], const int word_size){
...
void calc_stats(const char data[], double &mean, double &median, bool &is_binary){
...
void map_init(map<char, double> &m){
	for(int i = 0; i < 256; i++){
		m[i] = 0.0;
	}
}

// Map initialization for pair<char, char> to int
void map_init(map<pair<char, char>, int> &m){
	for(int i = 0; i < 256; i++){
		for(int j = 0; j < 256; j++){
			m[pair<char, char>(i,j)] = 0;
		}
	}
}

// Calculates proportions of each value as an index
void calc_proportions(const char data[], vector<double> &p){
	for(int i = 0; i < SIZE; i++){
		p[data[i]] += (1.0 / SIZE);
	}
}
...
vector<char> substr(const char text[], const int pos, const int len){
	int substr_len = len;
...
array<char, 16> fast_substr(const char text[], const int pos, const int len){
	array<char, 16> substring;
...
char max_map(const map<char, int> m){
	int max = -1;
	char key = 0;
